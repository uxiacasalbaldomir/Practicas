...
shinyServer(function(input, output) {
  ...
  # Display a numeric panel in case reference is custom
#  output$length <- renderUI({
#    switch(input$ref, "custom" =  numericInput("length", "Custom reference length", value = 3000000000))
#  })  

qc <- reactive ({
  qc_table<-input$qc
  qc<-fread(qc_table$datapath, header=TRUE, sep="\t", skip=1)
})
...
preseq <- reactive ({
  ...
  # removed 21/03/18
  #PreSeq<-cbind.data.frame(PreSeq[,1],PreSeq[,-1]/genome_territory())
})
